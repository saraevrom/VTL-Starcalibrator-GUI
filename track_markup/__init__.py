...
FORM_CONF = {
    "filter_win":{
        "type": "int",
        "default": 60,
        "display_name": "Окно фильтра"
    },
    "min_frame": {
        "type": "int",
        "default": 60,
        "display_name": "Минимальное количество кадров"
    },
    "start_skip":{
        "type": "int",
        "default": 0,
        "display_name": "Пропуск в начале"
    },
    "end_skip":{
        "type": "int",
        "default": 0,
        "display_name": "Пропуск в конце"
    },
    "pmt_select": {
        "type": "combo",
        "default": "full",
...
class TrackMarkup(ToolBase):
    def __init__(self, master):
        ...
        tk.Button(bottom_panel, text="Нифига не видно! Хочу перерисовать с другими параметрами",
                  command=self.redraw_event).pack(side="bottom", expand=True, fill="both")
        tk.Button(bottom_panel, text="Трек виден",
                  command=self.on_track_visible_poll).pack(side="left", expand=True, fill="both")
        tk.Button(bottom_panel, text="Трек не виден",
    ...
    def on_reset(self):
        if tkinter.messagebox.askokcancel("Сброс данных","Вы точно хотите стереть пустые интервалы и начать заново?"):
            self.reset_events()
    ...
    def show_next_event_it(self):
        if self.file:
            if not self.queue:
                tk.messagebox.showinfo("Разметка данных","Не осталось неразмеченных интервалов")
                return False
            form_data = self.params_form.get_values()
            win = form_data["filter_win"]
            print(self.queue)
